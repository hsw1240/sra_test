	...
	function startLoop() {
		$.ajax({
			...
			url: "http://210.92.123.150/monitoring/ftproot/F01",
		}).done(function(data) {
	...
	function showImg(num){
		console.log(num);
		num = num.replace(".txt", "");
		var pname = num.slice(0, -12);
		var drought = num.charAt(num.length-1);
		
		console.log(pname);
		
		//var wmsUrl = "http://210.92.123.148:8080/geoserver/sra/wms";
		var wmsUrl = "http://hygis-server.southeastasia.cloudapp.azure.com:8080/geoserver/SRA2017_SEP_F01/wms";
		if(ia_wms != undefined){
			ia_wms.destroy();
		}
		...
		$('#chartId').attr('src','http://210.92.123.150/monitoring/ftproot/F01/'+pname+'.wet_mask.chart.png');
