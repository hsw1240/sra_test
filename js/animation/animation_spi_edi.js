...
var attrD = "SPI";
var typeD = "TB_GEO_WKMBBSN";

loop("SPI", "TB_GEO_WKMBBSN");
...
function loop(layer, type) {
	...
	function spiFn() {
		...
		if(count < layerLen) {
			...
			showLayers(tempDate[count], "SPI", type);
			timeId = setTimeout(spiFn, 8000);
			count++;
		}else {
			
			count = 0;
			showLayers(tempDate[count], "SPI", type);
			timeId = setTimeout(ediFn, 8000);
			count++;
		}
	}
	...
	function ediFn() {
		if(count < layerLen) {
			showLayers(tempDate[count], "EDI", type);
			timeId = setTimeout(ediFn, 8000);
			count++;
		}else {
...
function showLayers(name, layer, type) {
	...
	if (type == "TB_GEO_WKMBBSN") {
		
		$("#dateTxt").text(layer + " / 대권역 / " + name);
		...
		wmsName = new OpenLayers.Layer.WMS(layer+" 대권역 "+name,
			wmsUrl,
			{
				layers: "drought_"+layer+"_layer",
				transparent: 'TRUE',
				format: 'image/png',
				viewparams: 'date:'+name+';area_tb:TB_GEO_WKMBBSN'
			}, {	
		...
		wmsName = new OpenLayers.Layer.WMS(layer+" 중권역 "+name,
			wmsUrl,
			{
				layers: "drought_"+layer+"_layer",
				transparent: 'TRUE',
				format: 'image/png',
				viewparams: 'date:'+name+';area_tb:TB_GEO_WKMMBSN'
			}, {	
		...
		wmsName = new OpenLayers.Layer.WMS(layer+" 행정구역 "+name,
			wmsUrl,
			{
				layers: "drought_"+layer+"_SIDO_layer",
				transparent: 'TRUE',
				format: 'image/png',
				viewparams: 'date:'+name+';area_tb:TB_GEO_Z_SOP_BND_SIDO_PG'
			}, {	
